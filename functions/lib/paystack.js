 ...
 */
export const paystackWebhook = onRequest({ secrets: [paystackSecretKey], cors: false }, async (req, res) => {
    if (req.method !== 'POST') {
 ...
 */
export const createPaystackCheckout = onCall({ secrets: [paystackSecretKey] }, async (request) => {
    if (!request.auth?.uid) {
