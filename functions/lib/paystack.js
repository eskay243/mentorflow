...
async function getApiKeys() {
    const snap = await firestoreDb().collection('config').doc('apiKeys').get();
    const data = snap.data() ?? {};
    const paystackSecretKey = data.paystackSecretKey ?? '';
    const paystackPublicKey = data.paystackPublicKey ?? '';
    if (!paystackSecretKey) {
        throw new HttpsError('failed-precondition', 'Paystack secret key not configured. Set it in Admin → Settings → API Keys.');
    }
    return { paystackSecretKey, paystackPublicKey };
}
/**
...
export const paystackWebhook = onRequest({ cors: false }, async (req, res) => {
    ...
    let secret;
    try {
        ({ paystackSecretKey: secret } = await getApiKeys());
    }
    catch {
    ...
    if (body.event === 'charge.success' && body.data?.reference) {
        ...
        if (enrollmentId && studentId) {
            ...
            if (enrollSnap.exists) {
                ...
                const commissionRate = (await db.collection('courses').doc(enroll.courseId).get()).data()
                    ?.commissionRate;
 ...
 */
export const createPaystackCheckout = onCall({}, async (request) => {
    if (!request.auth?.uid) {
        throw new HttpsError('unauthenticated', 'Sign in required');
    }
    const enrollmentId = request.data?.enrollmentId;
    const callbackUrl = request.data?.callbackUrl;
    if (!enrollmentId) {
        throw new HttpsError('invalid-argument', 'enrollmentId is required');
    }
    const { paystackSecretKey: secret } = await getApiKeys();
    const db = firestoreDb();
