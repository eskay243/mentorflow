...
async function getApiKeys(): Promise<{ paystackSecretKey: string; paystackPublicKey: string }> {
  const snap = await firestoreDb().collection('config').doc('apiKeys').get();
  const data = snap.data() ?? {};
  const paystackSecretKey = (data.paystackSecretKey as string | undefined) ?? '';
  const paystackPublicKey = (data.paystackPublicKey as string | undefined) ?? '';
  if (!paystackSecretKey) {
    throw new HttpsError(
      'failed-precondition',
      'Paystack secret key not configured. Set it in Admin → Settings → API Keys.',
    );
  }
